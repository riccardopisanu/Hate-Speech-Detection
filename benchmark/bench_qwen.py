...
CONFIGS = {
    "MAMI": {
        ...
        "prompts": [
            ...
            "Evaluate the memes image and caption together. If it expresses sexist or hateful content, output 1. If not, output 0.",
            ...
            "When interpreting the memes image and text jointly, does it demean or stereotype women? Answer 1 if yes, 0 if no.",
        ],
    },
    "MultiOFF": {
    ...
    "HatefulMemes": {
        ...
        "prompts": [
            ...
            "Evaluate the memes image and text together. Is it hateful? Output only 1 for hateful, 0 for not.",
...
def map_prediction(pred_text: str) -> int:
    ...
    if "cannot" in s or "sorry" in s:
        return 0
    if "not offensive" in s or "non-offensive" in s or "non-hateful" in s:
        return 0
        ...
        for p in cfg["prompts"]:
            try:
                query = tokenizer.from_list_format(
                    [{"image": img_path}, {"text": f"Text: {item['text']}\n\n{p}"}]
                )
