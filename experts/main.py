...
def patched_common_step(self, batch):
    ...
    text_tokens = batch["text"]  # [Batch, 77] (Gi tokenizzato dal dataset!)
    labels = batch["label"]
...
def main():
    ...
    parser.add_argument(
        "--config", type=str, default="", help="Path al file YAML di configurazione"
    )
    parser.add_argument(
        "--dataset", type=str, default="", help="Nome del dataset (opzionale, override)"
    )
    parser.add_argument(
        "opts",
        default=None,
        nargs=argparse.REMAINDER,
        help="Override opzioni YACS da riga di comando",
    )
    ...
    if cfg.model.name == "MemeCLIP":
        if MemeCLIP is None:
            raise ImportError(
                "Impossibile trovare MemeCLIP.py. Assicurati che sia nella cartella experts/"
            )
    ...
    if best_model_path:
        ...
        final_model = ModelClass.load_from_checkpoint(
            checkpoint_path=best_model_path, cfg=cfg
        )
