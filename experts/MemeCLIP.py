...
class MemeCLIP(pl.LightningModule):
    ...
    def __init__(self, cfg):
        ...
        drop_prob_1 = (
            cfg.drop_probs[1] if hasattr(cfg, "drop_probs") else cfg.model.drop_probs[1]
        )
        drop_prob_2 = (
            cfg.drop_probs[2] if hasattr(cfg, "drop_probs") else cfg.model.drop_probs[2]
        )
        ...
        unmapped_dim = (
            cfg.unmapped_dim if hasattr(cfg, "unmapped_dim") else cfg.model.unmapped_dim
        )
        num_mapping_layers = (
            cfg.num_mapping_layers
            if hasattr(cfg, "num_mapping_layers")
            else cfg.model.num_mapping_layers
        )

        self.image_map = LinearProjection(
            unmapped_dim, self.map_dim, num_mapping_layers, cfg.drop_probs
        )
        ...
        self.text_map = LinearProjection(
            real_text_dim, self.map_dim, num_mapping_layers, cfg.drop_probs
        )

        self.soft = nn.Softmax(dim=1)

        num_pre = (
            cfg.num_pre_output_layers
            if hasattr(cfg, "num_pre_output_layers")
            else cfg.model.num_pre_output_layers
        )
    ...
    def init_head_text_feat(self):
        ...
        c_names = (
            ...
            else self.cfg.model.class_names
        )
        ...
        prompts = clip.tokenize(
            [p for p in prompts], context_length=77, truncate=True
        ).to(device)
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log(
            f"train/auroc", output["auroc"], on_step=False, on_epoch=True, prog_bar=True
        )
        return total_loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log(
            f"val/auroc", output["auroc"], on_step=False, on_epoch=True, prog_bar=True
        )
    ...
    def configure_optimizers(self):
        ...
        wd = (
            ...
            if hasattr(self.cfg, "weight_decay")
            else self.cfg.training.weight_decay
        )
