...
def load_data(file_path):
    ...
    img = next(
        (
            cols[c]
            for c in ["file_name", "filename", "img", "image_name", "id"]
            if c in cols
        ),
        "img",
    )
...
if __name__ == "__main__":
    ...
    loader = DataLoader(
        SimpleDataset(df, args.image_root, preprocess),
        batch_size=32,
        num_workers=4,
        shuffle=False,
    ...
    pd.DataFrame({"label_true": all_labels, "zeroshot_pred": all_preds}).to_csv(
        f"results_zeroshot_{args.dataset_name}.csv", index=False
    )
