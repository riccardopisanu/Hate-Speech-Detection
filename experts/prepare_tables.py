...
def merge_dataset(name, files):
    ...
    if img_col != "image_name":
        df_nlp.rename(columns={img_col: "image_name"}, inplace=True)
    ...
    if os.path.exists(files["clip"]):
        ...
        if pred_col:
            df_clip = df_clip[["image_name", pred_col]].rename(
                columns={pred_col: "PRED_CLIP"}
            )
    ...
    if os.path.exists(files["meme"]):
        ...
        if pred_col:
            df_meme = df_meme[["image_name", pred_col]].rename(
                columns={pred_col: "PRED_MEME"}
            )
    ...
    for c in cols_to_fill:
        if c in df_nlp.columns:
            df_nlp[c] = df_nlp[c].fillna(-1).astype(int)

    if "label_true" in df_nlp.columns:
        df_nlp.rename(columns={"label_true": "true_label"}, inplace=True)
    elif "label" in df_nlp.columns:
        df_nlp.rename(columns={"label": "true_label"}, inplace=True)
