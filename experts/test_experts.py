...
class LinearProjection(nn.Module):
    def __init__(self, dim_in, dim_out, num_layers=2, drop_probs=[0.1] * 3):
        ...
        for _ in range(num_layers):
            layers.extend(
                [nn.Linear(dim_in, dim_out), nn.ReLU(), nn.Dropout(drop_probs[0])]
            )
            dim_in = dim_out
    ...
    def __init__(self, cfg):
        ...
        for _ in range(cfg.model.num_pre_output_layers):
            pre_output_layers.extend(
                [nn.Linear(self.map_dim, self.map_dim), nn.ReLU(), nn.Dropout(0.1)]
            )
...
class MultimodalDataset(Dataset):
    ...
    def _check_first_image(self):
        if len(self.df) > 0:
            ...
            if os.path.exists(cand):
                print(f"    FOUND AT: {cand}")
            else:
                print(f"    NOT FOUND AT: {cand}")
    ...
    def __getitem__(self, idx):
        ...
        try:
            image = self.preprocess(Image.open(final_path).convert("RGB"))
        except:
            return None
    ...
    if lbl is None:
        ...
        def clean_lbl(x):
            try:
                ...
                return (
                    1
                    if str(x).lower().strip() in ["yes", "1", "offensive", "misogynous"]
                    else 0
                )
...
def smart_load_weights(model, checkpoint_path, device):
    ...
    for k, v in state_dict.items():
        if k in model.state_dict():
            ...
            if k_fix in model.state_dict():
                new_state_dict[k_fix] = v
...
if __name__ == "__main__":
    ...
    if "memeclip" in args.model_type.lower():
        model = MemeCLIP(cfg)
    else:
    ...
    loader = DataLoader(
        MultimodalDataset(df, args.image_root, preprocess),
        batch_size=32,
        num_workers=4,
        shuffle=False,
    ...
    pd.DataFrame(
        {
            ...
            f"{args.model_type}_pred": all_preds,
            f"{args.model_type}_prob": all_probs,
        }
